...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding index on 'Timeseries', fields ['name']
        db.create_index(u'ddsc_core_timeseries', ['name'])

        # Adding unique constraint on 'SourceGroup', fields ['name']
        db.create_unique(u'ddsc_core_sourcegroup', ['name'])

        # Adding index on 'Source', fields ['name']
        db.create_index(u'ddsc_core_source', ['name'])

        # Adding unique constraint on 'TimeseriesGroup', fields ['name']
        db.create_unique(u'ddsc_core_timeseriesgroup', ['name'])

        # Adding index on 'Location', fields ['name']
        db.create_index(u'ddsc_core_location', ['name'])


    def backwards(self, orm):
        # Removing index on 'Location', fields ['name']
        db.delete_index(u'ddsc_core_location', ['name'])

        # Removing unique constraint on 'TimeseriesGroup', fields ['name']
        db.delete_unique(u'ddsc_core_timeseriesgroup', ['name'])

        # Removing index on 'Source', fields ['name']
        db.delete_index(u'ddsc_core_source', ['name'])

        # Removing unique constraint on 'SourceGroup', fields ['name']
        db.delete_unique(u'ddsc_core_sourcegroup', ['name'])

        # Removing index on 'Timeseries', fields ['name']
        db.delete_index(u'ddsc_core_timeseries', ['name'])
    ...
    models = {
        ...
        u'ddsc_core.location': {
            ...
            'created': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 5, 3, 0, 0)'}),
